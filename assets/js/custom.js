...
(function () {
  ...
  function updateInputDisplay(dates, instance) {
    if (dates.length === 2) {
      ...
      instance.input.value = `${startDateFormatted} a ${endDateFormatted}`;
    } else {
  ...
  function formatDate(dateString) {
    ...
    const month = date.toLocaleString('pt-BR', { month: 'short' }).replace('.', ''); // Get the short month name
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    for (let i = 0; i < genericExamples.length; ++i) {
      ...
      new Choices(element, {
        allowHTML: true,
        placeholderValue: "Este é um placeholder definido na configuração",
  ...
  const autoCompleteJS = new autoComplete({
    ...
    data: {
      src: [
        "Como as plantas se adaptam a diferentes ambientes?",
        "O que faz as marés do oceano subirem e descerem?",
        "Como o cérebro processa as emoções?",
        "Quais fatores contribuem para a formação de um arco-íris?",
        "Quem inventou o telefone?",
        "Qual o papel da Lua no ecossistema da Terra?",
        "Como os animais se comunicam entre si?",
        "O que causa os terremotos?",
        "Qual a importância da Grande Barreira de Corais?",
        "Como os ossos humanos se regeneram após uma lesão?"
      ],
      cache: true,
    },
    resultItem: {
